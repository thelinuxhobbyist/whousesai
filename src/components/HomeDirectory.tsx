...
export default function HomeDirectory({ entities }: { entities: Entity[] }) {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedType, setSelectedType] = useState<EntityType | 'all'>('all');

  const typedEntities = useMemo(
    () => (selectedType === 'all' ? entities : entities.filter((entity) => entity.type === selectedType)),
    [entities, selectedType]
  );

  const filteredEntities = typedEntities.filter((e) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10">
            ...
            <div className="bg-white border border-[#E3E5E9] rounded-[8px] p-[22px_24px] shadow-[0_1px_2px_rgba(30,42,58,0.05)]">
              <div className="flex justify-between items-center mb-[16px]">
                ...
                <Link href="/explore" className="text-[12.5px] text-[#3F4FBF] hover:underline shrink-0">View revision history</Link>
              </div>
