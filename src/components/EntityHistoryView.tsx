...
function initialSelection(revs: EntityRevision[]) {
  if (revs.length >= 2) {
    return { a: revs[1], b: revs[0] };
  }
  if (revs.length === 1) {
    return { a: null as EntityRevision | null, b: revs[0] };
  }
  return { a: null as EntityRevision | null, b: null as EntityRevision | null };
}

export default function EntityHistoryView({
  ...
  const slug = initialEntity.slug;
  const initial = initialSelection(initialRevisions);

  const [entity, setEntity] = useState(initialEntity);
  const [revisions, setRevisions] = useState(initialRevisions);
  const [reports, setReports] = useState(initialReports);

  const [selectedRevA, setSelectedRevA] = useState<EntityRevision | null>(initial.a);
  const [selectedRevB, setSelectedRevB] = useState<EntityRevision | null>(initial.b);
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (data.success) {
        ...
        const next = initialSelection(revs);
        setSelectedRevA(next.a);
        setSelectedRevB(next.b);
      }
    } catch (err) {
      console.error(err);
    }
