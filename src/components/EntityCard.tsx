...
export default function EntityCard({ entity }: EntityCardProps) {
  ...
  const visibleUses = uses.slice(0, 3);
  const extraUses = uses.length - visibleUses.length;
  const hasClaimsPreview = visibleTools.length > 0 || visibleUses.length > 0;

  return (
    <div className="group flex h-full flex-col rounded-[6px] bg-white border border-[#E3E5E9] p-5 sm:p-6 hover:border-[#3F4FBF] transition-all duration-200 shadow-[0_1px_2px_rgba(30,42,58,0.05)]">
      <div className="flex items-start justify-between gap-3">
        ...
        {rev && (
          <span className="flex items-center gap-1 text-[11px] mono text-[#3F4FBF] bg-[#F8F9FB] border border-[#E3E5E9] px-2 py-0.5 rounded shrink-0">
            <History className="w-3 h-3" /> Rev #{rev.revision_number}
          </span>
        )}
      </div>

      <div className="mt-3 flex-1 flex flex-col gap-4 min-h-0">
        <div>
          <Link href={`/entity/${entity.slug}`} className="block group-hover:text-[#3F4FBF] transition-colors">
            <h3 className="serif text-[20px] font-semibold text-[#1E2A3A] leading-tight flex items-start justify-between gap-2">
              <span>{entity.name}</span>
              <ArrowRight className="w-4 h-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-[#3F4FBF] shrink-0 mt-1" />
            </h3>
          </Link>
        ...
        {hasClaimsPreview && (
          <div className="rounded-[6px] bg-[#F8F9FB] border border-[#E3E5E9] p-3.5 space-y-3.5">
            {visibleTools.length > 0 && (
              <div>
                <span className="text-[10.5px] font-semibold text-[#3F4FBF] uppercase tracking-wider mb-2 flex items-center gap-1 font-sans">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {visibleTools.map((tool) => (
                    <span
                      key={tool}
                      className="mono px-2 py-0.5 text-[11.5px] rounded bg-white text-[#3F4FBF] border border-[#E3E5E9] font-medium"
                    >
                      {tool}
                    </span>
                  ))}
                  {extraTools > 0 && (
                    <span className="px-2 py-0.5 text-[11.5px] rounded bg-white text-[#8A93A3] border border-[#E3E5E9]">
                      +{extraTools} more
                    </span>
                  )}
                </div>
            ...
            {visibleUses.length > 0 && (
              <div>
                <span className="text-[10.5px] font-semibold text-[#8A93A3] uppercase tracking-wider mb-2 block font-sans">
                  Use cases
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {visibleUses.map((use, idx) => (
                    <span
                      key={`${use}-${idx}`}
                      className="px-2 py-0.5 text-[12px] rounded bg-white text-[#1E2A3A] border border-[#E3E5E9]"
                    >
                      {use}
                    </span>
                  ))}
      ...
      <div className="mt-5 pt-4 border-t border-[#E3E5E9] flex items-center justify-between gap-3 text-[12.5px] text-[#8A93A3] shrink-0">
